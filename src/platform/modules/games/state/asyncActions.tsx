...
const erc20TokenContractAbi = require("assets/json/Erc20abi.json");
...
export function createGame(stake: number, userSeed: string) {
    return async (dispatch: Dispatch, getState: GetState) => {
        ...
        const response = await axios.post("https://backdice.herokuapp.com/stateChannel/createGame");
        ...
        const userEndHash = hashChain[0];

        // This is about Approve
        var tokenContract = new web3.eth.Contract(erc20TokenContractAbi, tokenAddress);
        await tokenContract.methods.approve(contractAddress, fromGweiToWei(stake)).send({from: account}); // Change fromGweiToWei(stake)

        dispatch(createGameEvent(hashChain, serverEndHash, stake));

        console.log("Create Game", fromGweiToWei(stake));
        try {
            await new Promise((resolve, reject) => {
                createGame([userEndHash, serverEndHash], previousGameId, createBefore, signature, fromGweiToWei(stake)) // Change fromGweiToWei(stake)
                    .send({
                        from: account,
                        gas: 100000,
                    })
...
export function endGame() {
    return async (dispatch: Dispatch, getState: GetState) => {
        ...
        const {data} = await axios.post("https://backdice.herokuapp.com/stateChannel/endGame", {
            bet,
...
async function revealSeedRequest(gameId: number, roundId: number, userSeed: string, bet: object) {
    return retry(
        () => {
            return axios.post("https://backdice.herokuapp.com/stateChannel/revealSeed", {
                gameId,
                roundId,
                userSeed,
                bet,
...
export function requestSeed() {
    return async (dispatch: Dispatch, getState: GetState) => {
        ...
        const {data} = await revealSeedRequest(gameState.gameId, gameState.roundId, userSeed, {
            ...
            num,
            serverHash,
            serverSig: gameState.serverSig,
            userHash: gameState.userHash,
            userSig: gameState.userSig,
        });
...
export function placeBet(num: number, betValue: number, gameType: number) {
    return async (
        ...
        const {data: dataPlaceBet} = await axios.post("https://backdice.herokuapp.com/stateChannel/placeBet", {
            bet,
            contractAddress: CONTRACT_ADDRESS,
            // userSig,
        });
        ...
        dispatch(placeBetEvent(bet, serverSig, ""));
